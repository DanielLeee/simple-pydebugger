...
class debugger():
	...
	def __init__(self):
		...
		# Here let's determine and store the default page size for the system
	...
	def exception_handler_breakpoint(self):
		...
		# check if the breakpoint is one that we have
	...
	def enumerate_threads(self):
		...
		if snapshot is not None:
			...
			while success:
				...
				# print("%d %d" % (thread_entry.th32OwnerProcessID, thread_entry.th32ThreadID))
	...
	def read_process_memory(self, address, length):
		...
		print("read process memory %r, 0x%08x, %r, %r, %r" % (self.h_process, address, read_buf, length, byref(count)))

		temb = kernel32.ReadProcessMemory(self.h_process, address, read_buf, length, byref(count))

		print("read process memory returns: %r" % temb)

		if temb == 0:

			print("read process memory false")
			return False
			...
			print("read process memory true")
			return data
	...
	def bp_set(self, address):
		...
		if not address in self.breakpoints:
			...
			try:
				...
				print("[*] In trying.")
				# store thr original byte
				original_byte = self.read_process_memory(address, int(1))

				print("original_byte %r" % original_byte)
